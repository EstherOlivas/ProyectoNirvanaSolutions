const tbodyPermisos = document.querySelector('#informacion-registro tbody')
const btn_RegistrarPermisos = document.getElementById('btnRegistrarPermisos')
...
const mostrar_datos_en_tabla = async () => {
  ...
  for (let i = 0; i < listado_de_permisos.length; i++) {
    let fila = tbodyPermisos.insertRow()

    let celda_nombre = (fila.insertCell().innerHTML =
      listado_de_permisos[i]['nombre'])
  }
}
...
btn_RegistrarPermisos.addEventListener('click', obtener_datos)
